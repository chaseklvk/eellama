...
class Attention(nn.Module):
    def __init__(self, args: ModelArgs):
		...
		# for 1 node, n_local_heads = n_heads
...
class TailTransformer(nn.Module):
    ...
    def forward(self, h: torch.Tensor, start_pos: int):
        ...
        ### BEGIN Precompute steps required for both head and tail models
        self.freqs_cis = self.freqs_cis.to(h.device)
        freqs_cis = self.freqs_cis[start_pos : start_pos + seqlen]

        #mask = None
        #if seqlen > 1:
        # NOTE: this above is causing an error when using JIT, for now assume seq_len always > 1
